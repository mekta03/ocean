...
def create_map_levels(df, min_yrs, max_yrs):
    ...
    fig_map_all.write_html(
        f'{filename}/{name_project_files}/{min_lvl_name}_{max_lvl_name}m__coord_{min_lat}_{max_lat}.html',
        auto_open=True)
...
def scatter_new(df, lvl):
    ...
    fig.write_html(f'{filename}/{name_project_files}/{lvl}_m.html', auto_open=True)
...
def mean_for_nst_year_decade(df, year, name_of_decade):
    ...
    # print('ПРОВЕРКА')
        ...
        # dct_nst_means = {}
        # for nst in dff['Stations'].unique():
        #     dct_nst_means[nst] = df_all_nst_for_year[f'nst_{nst}'].mean()
...
def mean_year_decade_to_std_lvl(start_dec, end_dec):
    ...
    for year in sorted(df_new['Year'].unique()):
        ...
        dff_new_mean = pd.merge(dff_new_mean, dff_new_mean_2, on='level')
...
def graph_profile_of_means():
    ...
    for k, v in dct_years.items():
        ...
        df_for_means_graph = pd.merge(df_for_means_graph, df, on=['level'])
    ...
    if create_graph:
        ...
        fig_graph.write_html(f'{filename}/{name_project_files}/{min_lat}_{parameter}_profiles.html', auto_open=True)
